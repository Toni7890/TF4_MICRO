...
/****************************************************************
 * ESTRUCTURA DE LA FSM GENERADA POR MCC CLASSIC                *
 ****************************************************************/
...
/*****************************************************************************
 *          FUNCIONES TOMADAS DE LOS EJEMPLOS DE MCC CLASSIC                 *
 *****************************************************************************/
typedef struct
{
    size_t len;
...
void I2C1_Write(i2c1_address_t address, uint8_t data) {
    ...
    while(!I2C1_Open(address)); // Esperar hasta que el bus est? disponible

    // Configurar la callback y el buffer sin especificar un registro
    I2C1_SetDataCompleteCallback(wr1RegCompleteHandler, &data); // Callback para cuando se complete la escritura
    I2C1_SetBuffer(&data, 1); // Usar la direcci?n de la variable est?tica
    I2C1_SetAddressNackCallback(NULL, NULL); // Callback para manejar NACK, no se utiliza en este ejemplo

    I2C1_MasterWrite(); // Iniciar la escritura del dato
    while(I2C1_BUSY == I2C1_Close()); // Esperar hasta que la transacci?n haya finalizado
}
